...
/// Unity 风格 Animator 窗口：独立 Dock 面板。
/// 布局：中央状态机图（AnimatorGraphView）+ 右侧参数面板 + 底部属性面板。
/// - 图中右键拖拽建转换、拖节点移动、Delete 删除选中、滚轮缩放
/// - 参数面板：增删 float/bool/trigger 参数
/// - 属性面板：选中状态 → 名称/入口 + .anim 剪辑资产（必选）；选中转换 → from/to + 条件列表
/// 每个状态必须引用 .anim 资产（Unity 语义），内嵌剪辑编辑已移除；
/// .anim 文件在 Animation 窗口编辑。所有修改经 Animator API 写回并同步载体；
/// setGameObject 绑定/解绑，每帧 refresh()。
...
private:
	...
	/// 设置当前状态资产路径（写 assetPath + 更新 clip）并刷新
